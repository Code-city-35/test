// ============================================================
//  box.js — страница квеста (рабочая версия)
// ============================================================
...
(function injectStyles() {
    ...
    style.textContent = `
        ...
        .question-image { max-width: 100%; max-height: 200px; margin-bottom: 10px; border-radius: 4px; }
    `;
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (boxId) {
        try {
            ...
            if (userId) {
                // Проверяем, проходил ли квест ранее
                const hasCompleted = await checkIfCompleted(boxId);
                isFirstTime = !hasCompleted;
                console.log('isFirstTime:', isFirstTime);
            }

            // Создаём или находим попытку
...
// ============================================================
//  ПРОВЕРКА, ПРОХОДИЛ ЛИ УЖЕ КВЕСТ
// ============================================================
async function checkIfCompleted(questId) {
    try {
...
async function createQuestAttempt(questId) {
    ...
    try {
        ...
        if (inProgress) {
            ...
            currentStep = inProgress.current_clue_index || 0;
            console.log('✅ Продолжаем попытку:', attemptId, 'шаг:', currentStep);
            return;
        }

        // 2. Создаём новую попытку
        ...
        currentStep = 0;
        console.log('✅ Создана новая попытка:', attemptId);
    } catch (e) {
...
async function addXP(userId, xpAmount) {
    ...
    try {
        ...
        if (leveledUp) console.log(`🎉 Уровень повышен до ${newLevel}!`);
    } catch (e) {
...
async function completeQuestAttempt() {
    if (!attemptId) {
        console.warn('⚠️ Нет attemptId, пропускаем завершение');
        return;
    }
    ...
    try {
        ...
        if (error) throw error;
        console.log('✅ Квест успешно завершён в БД');
...
function showStep(index) {
    ...
    html += `
        ...
        <div id="questionBlock" style="display:none; margin-top:16px; border-top:1px solid #2a2a2a; padding-top:12px;">
            ${clue.question_image ? `<img src="${clue.question_image}" alt="Изображение к вопросу" class="question-image">` : ''}
...
function finishBox() {
    ...
    if (attemptId) {
        ...
        console.warn('⚠️ Нет attemptId, завершение невозможно');
    }
    ...
    container.innerHTML = `
        <div class="clue-block">
            ...
            <div class="clue-content">
                <p style="font-size: 20px; color: #ffb000;">Вы прошли квест!</p>
                <p></p>
                <p style="font-size: 24px; color: #00d4ff;">${currentBox.final_coords || 'не указаны'}</p>
                <p style="color: #8aa3c0; font-size: 14px;">Можешь попробовать пройти другие наши квесты!</p>
                <p style="margin-top: 12px;">⏱️ Ваше время: ${formatTime(timerSeconds)}</p>
                <p style="color: #6fcf97; margin-top: 8px;">✅ Квест пройден!</p>
